...
RSpec.describe Executors::Ollama do
  describe '#call' do
    ...
    let(:executor) { Executors::Ollama.new(model_version) }
    ...
    context 'when HTTP request is successful' do
      before do
        stub_request(:post, 'http://example.com/model_endpoint')
          ...
          .to_return(status: 200, body: [{ response: 'Test response' }, { response: 'Test response' }].map(&:to_json).join("\n"), headers: {})
      end

      it 'returns a completed status with result on successful response' do
        expect(executor.call(prompt)).to eq({ status: :completed, result: '[{"response":"Test response"},{"response":"Test response"}]' })
      end
    end
