...
RSpec.describe Executors::Openai do
  ...
  let(:model_version) { create(:model_version, model: model, configuration: { param: "value" }) }
  ...
  describe "#call" do
    ...
    context "when OpenAI API call raises an error" do
      before do
        allow_any_instance_of(OpenAI::Client).to receive(:chat).and_raise(Faraday::ClientError.new("API error", body: { error_message: "API error" } ))
      end
